 ...
 * Harness — 种子数据生成入口
 */
import { createSeedData } from './factories';
...
export function generateSeedData() {
  ...
  writeFileSync(join(fixturesDir, 'admin.json'), JSON.stringify(seed.admin, null, 2));
  writeFileSync(join(fixturesDir, 'projects.json'), JSON.stringify(seed.projects, null, 2));
  writeFileSync(join(fixturesDir, 'login-response.json'), JSON.stringify(seed.loginResponse, null, 2));

  console.log('Seed data generated:');
  console.log(`  - ${join(fixturesDir, 'admin.json')}`);
  console.log(`  - ${join(fixturesDir, 'projects.json')}`);
  console.log(`  - ${join(fixturesDir, 'login-response.json')}`);
