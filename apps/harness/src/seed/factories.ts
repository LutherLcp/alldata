...
export function createFunnelData(steps = 5): AnalysisResult {
  ...
  for (let i = 0; i < Math.min(steps, stepNames.length); i++) {
    if (i > 0) total = Math.round(total * faker.number.float({ min: 0.4, max: 0.85 }));
    rows.push({
      step: stepNames[i],
      value: total,
      rate: i === 0 ? 1 : total / firstTotal,
    });
  }
  ...
  return {
    ...
    rows,
    chart_data: [{ name: '漏斗', data: rows.map((r) => ({ x: r.step as string, y: r.value as number })) }],
  };
}
...
export function createDownloadTask(overrides: Partial<DownloadTask> = {}): DownloadTask {
  const status = faker.helpers.arrayElement([1, 2, 3, 4]) as 1 | 2 | 3 | 4;
  return {
    id: nextId(),
    project_id: 1,
